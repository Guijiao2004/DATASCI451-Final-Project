...
cor_data <- olympics[, c("X.Athletes", "X.Events", "IsHost")]
...
```
```{r}
log_cor_data <- olympics[, c("log_X.Athletes", "log_X.Events", "IsHost")]

log_cor_matrix <- cor(log_cor_data)

print(log_cor_matrix)

corrplot(log_cor_matrix, method = "color", addCoef.col = "black")

```
```{r}
library(car)
car::vif(lm(Total ~ log(X.Athletes+1) + log(X.Events+1), data = olympics))
...
pred_data <- function(data, Country, IsHost) {
  ...
  pred = data.frame(
    Year = 34,
    # + 1 to not explode
    log_X.Athletes = log(predict(lm.athletes, newdata = data.frame(Year = 34)) + 1),
    #X.Events = predict(lm.events, newdata = data.frame(Year = 34)),
    IsHost = IsHost,
    Country = Country
  )
  return(pred)
...
post_pred <- function(fit, new_data, country_id) {
  # Extract posterior samples
  posterior_samples <- extract(fit)
  alpha_samples <- posterior_samples$alpha[, country_id]    # vector of MCMC samples for alpha[c]
...
predict_for_country <- function(
  ...
  gold_pred   <- post_pred(gold_fit, new_data, country_id)
  silver_pred <- post_pred(silver_fit, new_data, country_id)
  bronze_pred <- post_pred(bronze_fit, new_data, country_id)
