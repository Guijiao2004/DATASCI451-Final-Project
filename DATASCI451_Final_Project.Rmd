...
$$
$$
\log(\lambda_{c, y}) = \alpha_c + \beta_0 + \beta_1\text{X.Athletes}_{c, y} + \beta_2\text{X.Events}_{c, y} + \beta_3\text{IsHost}_{c, y} + \beta_4\text{Year}_{c, y}
$$
$$
...
parameters {
  ...
  array[5] real beta; // coefficients for predictors
}
model {
  for (c in 1:C) {
    for (y in 1:Y) {
      Total[c, y] ~ poisson_log(alpha[c] + beta[1] + beta[2] * N_athletes[c, y] + beta[3] * N_events[c, y] + beta[4] * IsHost[c, y]);
    }
  }
...
total_pred <- rpois(length(lambda_pred), lambda_pred)
```
